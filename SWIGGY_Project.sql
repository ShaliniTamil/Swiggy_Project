...
CREATE TABLE DeliveryPartnersmov_collect
(
...
(
    ...
    FOREIGN KEY(PartnerID)
        REFERENCES DeliveryPartnersmov_collect(PartnerID)
);
...
INSERT INTO DeliveryPartnersmov_collect
...
-- 15. menu items costing less than ₹200.
select*
from menuitems
where Price < 200;
...
SELECT ITEMNAME AS FOODITEM
FROM MENUITEMS;

-- 39.Display menu prices after adding a 5% service charge.
SELECT PRICE*1.05 AS SERVICE_CHARGE
FROM MENUITEMS;
...
SELECT AVG(TOTALAMOUNT) AS AVG_AMT
FROM ORDERS;
...
SELECT AVG((FOODRATING + DELIVERYRATING)/2) AS AVG_RATING
FROM REVIEWS;

-- 50.Display the average review rating rounded to two decimal places.
SELECT ROUND(AVG((FOODRATING + DELIVERYRATING)/2),2) AS AVG_RATING
FROM REVIEWS;
...
SELECT PAYMENTMETHOD, COUNT(TRANSACTIONID) AS NO_OF_TRANSACTION
FROM PAYMENTS
GROUP BY PAYMENTMETHOD;
...
SELECT CITY, COUNT(*) AS REG_CUSTOMER
FROM CUSTOMERS
GROUP BY CITY
HAVING REG_CUSTOMER > 5;
...
SELECT RN.RESTAURANTNAME,
	R.DELIVERYRATING,
    R.REVIEWCOMMENT
FROM RESTAURANTS RN 
INNER JOIN REVIEWS R
ON RN.RESTAURANTID = R.RESTAURANTID;
...
ON RN.RESTAURANTID = MI.RESTAURANTID;
...
SELECT
r.RestaurantName,
AVG(FoodRating+DeliveryRating) as avg_rating
FROM restaurants as r
INNER JOIN reviews as rv
...
SELECT
dp.PartnerName,
COUNT(DeliveryID)As total_delivery
...
#88 Display restaurant name, city, and average menu price.
 SELECT
 r.RestaurantName,
 AVG(price)as average_menu_price
 FROM restaurants as r
...
INNER JOIN Delivery D
    ON O.OrderID = D.OrderID
INNER JOIN DeliveryPartnersmov_collect DP
    ON D.PartnerID = DP.PartnerID;
...
SELECT ORDERID,YEAR(ORDERDATE) AS DELIVERY_YR
FROM ORDERS;

-- 98. Display the order month for every order. 
SELECT ORDERID,MONTH(ORDERDATE) AS DELIVERY_MONTH
FROM ORDERS;

-- 99. Display the month name for every order. 
SELECT ORDERID,MONTHNAME(ORDERDATE) AS DELIVERY_MONTH
FROM ORDERS;

-- 100. Display the day of the month for every order. 
SELECT ORDERID,DAY(ORDERDATE) AS DELIVERY_DAY,MONTHNAME(ORDERDATE) AS DELIVERY_MONTH,DAYNAME(ORDERDATE) AS DELIVERY_DAY
FROM ORDERS;

-- 101. Display the weekday name for every order. 
SELECT ORDERID,DAYNAME(ORDERDATE) AS DELIVERY_DAY
FROM ORDERS;

-- 102. Display the weekday number for every order.
SELECT ORDERID,WEEKDAY(ORDERDATE) AS DELIVERY_WEEK
FROM ORDERS;
 
-- 103. Display the week number for every order. 
SELECT ORDERID,WEEK(ORDERDATE) AS DELIVERY_WEEK
FROM ORDERS;
...
SELECT ORDERID,DAYOFYEAR(ORDERDATE) 
FROM ORDERS;

-- 106. Calculate the number of days between the order date and delivery date. 
SELECT ORDERID, ORDERDATE,ESTIMATEDDELIVERY,DATEDIFF(ESTIMATEDDELIVERY,ORDERDATE )AS DAY 
FROM ORDERS;

-- 107. Calculate the delivery duration in minutes. 
SELECT ORDERID,MINUTE(ESTIMATEDDELIVERY)
FROM ORDERS;

-- 108. Display the expected delivery date by adding two days to the order date.
SELECT ORDERID,ESTIMATEDDELIVERY,ADDDATE(ESTIMATEDDELIVERY, INTERVAL 2 DAY) AS 2_DAYS_ADDED
FROM ORDERS;
 
-- 109. Display a reminder date three days before the order date. 
SELECT ORDERID,ORDERDATE,SUBDATE(ORDERDATE, INTERVAL 3 DAY) AS REMINDER_DATE
FROM ORDERS;

-- 110. Add seven days to each order date. 
SELECT ORDERID,ADDDATE( ORDERDATE,INTERVAL 7 DAY) AS 7_DAYS_ADDED
FROM ORDERS;

#111 Subtract five days from each order date. 
SELECT ORDERID,ORDERDATE,SUBDATE(ORDERDATE, INTERVAL 5 DAY) AS 5_DAYS_SUBTRACTED
FROM ORDERS;
...
SELECT ORDERID,DATE_FORMAT(ORDERDATE, '%D-%M-%Y') AS ORDER_DATE
FROM ORDERS;

#114 Display the order month and year in "Month YYYY" format. 
SELECT ORDERID,DATE_FORMAT(ORDERDATE, '%M-%Y') AS ORDER_DATE
FROM ORDERS;
...
SELECT AVG(TIMESTAMPDIFF(MINUTE, ORDERDATE, ESTIMATEDDELIVERY)) AS AVG_DELIVERY_TIME
FROM ORDERS;
...
SELECT C.CustomerID,
       ...
       ROW_NUMBER() OVER (ORDER BY SUM(O.TotalAmount) DESC) AS Row_Num
FROM Customers C
JOIN Orders O
ON C.CustomerID = O.CustomerID
GROUP BY C.CustomerID;
...
SELECT
    ...
    DENSE_RANK() OVER (ORDER BY SUM(O.TOTALAMOUNT) DESC) AS SPENDINGRANK
FROM CUSTOMERS C
JOIN ORDERS O
    ON C.CUSTOMERID = O.CUSTOMERID
GROUP BY
...
SELECT
    ...
    CUSTOMERNAME,
    LIFETIMESPENDING,
    NTILE(4) OVER (ORDER BY LIFETIMESPENDING DESC) AS SPENDINGGROUP
FROM
(
    SELECT
        C.CUSTOMERID,
...
SELECT
    ...
    PAYMENTDATE,
    AMOUNT,
    LAG(AMOUNT) OVER (ORDER BY PAYMENTDATE) AS PREVIOUSPAYMENT
FROM PAYMENTS;
...
SELECT
    ...
    PAYMENTDATE,
    AMOUNT,
    LEAD(AMOUNT) OVER (ORDER BY PAYMENTDATE) AS NEXTPAYMENT
FROM PAYMENTS;
...
SELECT
    COUNT(CASE WHEN PAYMENTSTATUS = 'SUCCESS' THEN 1 END) AS COMPLETED,
    COUNT(CASE WHEN PAYMENTSTATUS = 'PENDING' THEN 1 END) AS PENDING,
    COUNT(CASE WHEN PAYMENTSTATUS = 'FAILED' THEN 1 END) AS FAILED,
...
SELECT
    R.RESTAURANTNAME,
    AVG(RV.FOODRATING) AS AVERAGERATING
FROM RESTAURANTS R
JOIN REVIEWS RV
    ON R.RESTAURANTID = RV.RESTAURANTID
GROUP BY
...
SELECT
    DP.PARTNERNAME,
    AVG(TIMESTAMPDIFF(MINUTE, O.ORDERDATE, O.ESTIMATEDDELIVERY)) AS AVERAGEDELIVERYTIME
...
SELECT
    ...
    RANK() OVER (ORDER BY COUNT(*) DESC) AS PAYMENTRANK
FROM PAYMENTS
WHERE PAYMENTSTATUS = 'SUCCESS'
GROUP BY PAYMENTMETHOD;

#146 DISPLAY EACH RESTAURANT REVENUE ALONG WITH ITS PERCENTAGE CONTRIBUTION TO TOTAL REVENUE.
SELECT
    R.RESTAURANTNAME,
    SUM(O.TOTALAMOUNT) AS TOTALREVENUE,
    ROUND(
...
SELECT
    C.CUSTOMERID,
    CONCAT(C.FIRSTNAME, ' ', C.LASTNAME) AS CUSTOMERNAME,
    COUNT(DISTINCT O.RESTAURANTID) AS RESTAURANTCOUNT
FROM CUSTOMERS C
JOIN ORDERS O
    ON C.CUSTOMERID = O.CUSTOMERID
GROUP BY
...
SELECT
    ...
    (SELECT COUNT(*) FROM PAYMENTS WHERE PAYMENTSTATUS = 'SUCCESS') AS COMPLETEDPAYMENTS,
